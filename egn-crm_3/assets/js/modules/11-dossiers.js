...
function renderDetailCharges(T,eur){
  ...
  h+=`<div style="display:flex;gap:.35rem;align-items:center;flex-wrap:wrap;margin-bottom:.8rem">
    ...
    ${POSTES_CHARGES.map(p=>{
      const n=toutes.filter(x=>x[p.cle]>0).length;
      return btn(`${p.lab} <span style="opacity:.7">${n}</span>`,CHARGES_POSTE===p.cle,`setChargesPoste('${p.cle}')`);
    }).join('')}
    ${(CHARGES_POSTE&&POSTE_VERS_TYPE[CHARGES_POSTE])?(()=>{
      // Intervenants presents sur ce poste, avec leur montant cumule
      const parAgent={};
      toutes.filter(x=>x[CHARGES_POSTE]>0).forEach(x=>{
      ...
      return `<select onchange="setChargesAgent(this.value)" style="font-family:'Saira',sans-serif;font-size:.78rem;font-weight:700;border:1.5px solid var(--border-grey);border-radius:8px;padding:.32rem .6rem;background:#fff;outline:none">
        <option value="">Tous les intervenants</option>
        ${liste.map(([id,a])=>`<option value="${id==='__sans__'?'':id}" ${CHARGES_AGENT===id?'selected':''}>${esc(a.nom)} — ${eur(a.total)} (${a.nb})</option>`).join('')}
      </select>`;
    })():''}
    ${(CHARGES_POSTE==='ma'&&fournisseurs.length)?`<select onchange="setChargesFourn(this.value)" style="font-family:'Saira',sans-serif;font-size:.78rem;font-weight:700;border:1.5px solid var(--border-grey);border-radius:8px;padding:.32rem .6rem;background:#fff;outline:none">
