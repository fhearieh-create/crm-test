...
function renderComptaPaiements(){
  ...
  filtered.forEach(f=>{
    ...
    const pieces=piecesApf(f);
    const pjCell=`<div style="display:flex;flex-direction:column;gap:2px">
      ${pieces.map(x=>`<div style="display:flex;align-items:center;gap:.25rem">${attachLink(x.path,x.name||'Voir',x.url)}${!locked?`<button class="iconbtn del" style="font-size:.6rem;padding:0 3px" title="Retirer" onclick="deletePaiementApfFile('${f.id}','${esc(x.path)}')">✕</button>`:''}</div>`).join('')}
      ${!locked?`<label style="cursor:pointer;display:inline-flex;align-items:center;gap:.2rem;font-size:.72rem;font-weight:600;color:var(--blue)"><input type="file" accept=".pdf,image/*" multiple style="display:none" onchange="uploadPaiementApfFile('${f.id}',this)">📎 ${pieces.length?'Ajouter':'Joindre'}</label>`:(pieces.length?'':`<span class="muted" style="font-size:.72rem">—</span>`)}
    </div>`;
...
async function supprimerApfPaiement(factureId){
  modalConfirm('Supprimer cet APF ?','Les dossiers concernés repasseront en "non facturés" et pourront être refacturés.',async()=>{
    ...
    if(f){
      ...
      // L'acompte impute sur cet APF redevient disponible pour l'intervenant
      const restitue=restituerAcompte(factureId);
      await deleteFactureRemote(factureId);
      FACTURES_CACHE=FACTURES_CACHE.filter(x=>x.id!==factureId);
      if(restitue>0)toast(`Acompte restitué : ${restitue.toLocaleString('fr-FR',{minimumFractionDigits:2})} € de nouveau disponible`,'ok');
    }
...
/* Pieces jointes d'un APF : liste unifiee, avec reprise de l'ancien
   champ unique (fileUrl/filePath) pour les APF deja enregistres */
function piecesApf(f){
  const p=f.paiements||{};
  const l=Array.isArray(p.pieces)?p.pieces.slice():[];
  if(p.filePath&&!l.some(x=>x.path===p.filePath))l.unshift({path:p.filePath,name:p.fileName||'Facture',url:p.fileUrl||''});
  return l;
}
async function uploadPaiementApfFile(factureId,input){
  const fichiers=[...(input.files||[])];if(!fichiers.length)return;
  const f=FACTURES_CACHE&&FACTURES_CACHE.find(x=>x.id===factureId);if(!f)return;
  f.paiements=f.paiements||{};
  f.paiements.pieces=piecesApf(f);
  // L'ancien champ unique est desormais porte par la liste
  delete f.paiements.fileUrl;delete f.paiements.filePath;delete f.paiements.fileName;
  let ok=0;
  for(const file of fichiers){
    if(file.size>10*1024*1024){toast(file.name+' : trop lourd (max 10 Mo)','err');continue;}
    try{
      ...
      f.paiements.pieces.push({path,name:file.name,url:'',date:Date.now()});
      ok++;
    }catch(e){console.error('[upload]',e);toast(file.name+' : '+(e.message||JSON.stringify(e)),'err');}
  }
  input.value='';
  if(ok){await persistFacture(f);toast(`${ok} pièce${ok>1?'s':''} jointe${ok>1?'s':''} ✓`,'ok');}
  renderComptaPaiements();
}
async function deletePaiementApfFile(factureId,path){
  const f=FACTURES_CACHE&&FACTURES_CACHE.find(x=>x.id===factureId);if(!f)return;
  const l=piecesApf(f);
  const cible=path?l.find(x=>x.path===path):l[0];
  if(cible){try{await SUPA.storage.from(ATTACH_BUCKET).remove([cible.path]);}catch(e){}}
  f.paiements=f.paiements||{};
  f.paiements.pieces=l.filter(x=>x!==cible);
  delete f.paiements.fileUrl;delete f.paiements.filePath;delete f.paiements.fileName;
