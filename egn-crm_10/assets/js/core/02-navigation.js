/* ============================================================
   ...
   Toast, menu, pages, routeur, recherche globale, notifications, journal d'activite
   ============================================================ */
/* ====================================================================
   NAVIGATION & ROUTEUR
   ==================================================================== */
...
/* ---- recherche globale ---- */
let GSEARCH_T=null;
let GSEARCH_RES=[], GSEARCH_SEL=0;
function globalSearch(q){
  clearTimeout(GSEARCH_T);
  const box=document.getElementById('gsearchResults');
  if(!q||q.trim().length<2){box.style.display='none';GSEARCH_RES=[];return;}
  GSEARCH_T=setTimeout(()=>{
    ...
    GSEARCH_RES=recs;GSEARCH_SEL=0;
    if(!recs.length){box.innerHTML='<div class="notif-empty">Aucun résultat</div>';box.style.display='block';return;}
    box.innerHTML=recs.map((r,i)=>{const op=getAllProducts().find(o=>o.id===r.productId);return `<div class="gsearch-item${i===GSEARCH_SEL?' gs-sel':''}" data-gs="${i}" onmouseenter="GSEARCH_SEL=${i};majSelectionGsearch()" onclick="closeGsearch();openDossier('${r.id}')">
...
/* Surligne le resultat courant et le garde visible */
function majSelectionGsearch(){
  const box=document.getElementById('gsearchResults');if(!box)return;
  [...box.querySelectorAll('.gsearch-item')].forEach((el,i)=>{
    el.classList.toggle('gs-sel',i===GSEARCH_SEL);
    if(i===GSEARCH_SEL&&el.scrollIntoView)el.scrollIntoView({block:'nearest'});
  });
}
/* Flèches pour parcourir, Entrée pour ouvrir, Échap pour fermer */
function gsearchClavier(ev){
  const box=document.getElementById('gsearchResults');
  const ouvert=box&&box.style.display==='block'&&GSEARCH_RES.length;
  if(ev.key==='Escape'){closeGsearch();return;}
  if(!ouvert){
    // Entrée avant l'affichage des résultats : on lance la recherche sans attendre
    if(ev.key==='Enter'){ev.preventDefault();clearTimeout(GSEARCH_T);globalSearch(ev.target.value);}
    return;
  }
  if(ev.key==='ArrowDown'){ev.preventDefault();GSEARCH_SEL=(GSEARCH_SEL+1)%GSEARCH_RES.length;majSelectionGsearch();}
  else if(ev.key==='ArrowUp'){ev.preventDefault();GSEARCH_SEL=(GSEARCH_SEL-1+GSEARCH_RES.length)%GSEARCH_RES.length;majSelectionGsearch();}
  else if(ev.key==='Enter'){
    ev.preventDefault();
    const r=GSEARCH_RES[GSEARCH_SEL];
    if(r){closeGsearch();openDossier(r.id);}
  }
}
function closeGsearch(){GSEARCH_RES=[];GSEARCH_SEL=0;const box=document.getElementById('gsearchResults');if(box)box.style.display='none';const i=document.getElementById('gsearchInput');if(i)i.value='';}
