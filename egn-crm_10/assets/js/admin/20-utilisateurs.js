...
/* Extrait le vrai message d'erreur renvoye par une fonction serveur.
   Sans cela, supabase-js ne remonte que « non-2xx status code ». */
async function messageErreurFonction(error){
  try{
    const rep=error&&error.context;
    if(rep&&typeof rep.json==='function'){
      const d=await rep.clone().json();
      if(d&&d.error)return d.error;
      if(d&&d.message)return d.message;
    }
    if(rep&&typeof rep.text==='function'){
      const t=await rep.clone().text();
