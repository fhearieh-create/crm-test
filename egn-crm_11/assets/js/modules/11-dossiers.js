...
/* Annule les imputations liees a un APF supprime : les consommations sont
   retirees du journal, le solde de l'intervenant remonte d'autant. */
function restituerAcompte(factureId){
  const l=getAcomptes();
  const liees=l.filter(a=>a.nature==='consommation'&&a.factureId===factureId);
  if(!liees.length)return 0;
  const montant=liees.reduce((s,a)=>s+(a.montant||0),0);
  saveAcomptes(l.filter(a=>!(a.nature==='consommation'&&a.factureId===factureId)));
  liees.forEach(a=>deleteRemoteRow('egncrm_acomptes',a.id));
...
let FACT_TVA=''; // regime TVA impose a l'APF ('' = selon chaque dossier)
  ...
  if(peut){
    ...
    h+=`<div class="panel"><div class="panel-h"><h3>Verser un acompte</h3></div><div class="panel-b">
      <div class="fgrid">
        ...
        <div class="fld"><label>Régime de TVA <span class="muted" style="font-size:.62rem;text-transform:none;letter-spacing:0">du montant saisi</span></label>
          <select id="ac_tva" style="${inp}">
            <option value="ttc">TTC — TVA incluse</option>
            <option value="ht">HT — TVA en plus, récupérable</option>
            <option value="sans_tva">Sans TVA — autoentrepreneur</option>
          </select></div>
        <div class="fld"><label>Justificatif <span class="muted" style="font-size:.62rem;text-transform:none;letter-spacing:0">facture, reçu — PDF ou image</span></label>
          <input type="file" id="ac_fichier" accept=".pdf,image/*" style="font-size:.8rem;padding:.3rem 0"></div>
      </div>
...
/* Decomposition HT / TVA / TTC d'un acompte selon son regime.
   Le solde imputable est toujours le TTC : c'est ce qui a ete decaisse. */
function tvaAcompte(a){
  const t=tvaRate()/100, m=a.montant||0, r2=v=>Math.round(v*100)/100;
  const mode=a.tvaMode||'ttc';
  if(mode==='ht')return{mode,ht:m,tva:r2(m*t),ttc:r2(m*(1+t)),recup:true};
  if(mode==='sans_tva')return{mode,ht:m,tva:0,ttc:m,recup:false};
  const ht=t?r2(m/(1+t)):m;
  return{mode,ht,tva:r2(m-ht),ttc:m,recup:false};
}
async function enregistrerAcompte(){
  const type=(document.getElementById('ac_type')||{}).value||'';
  const agentId=(document.getElementById('ac_agent')||{}).value||'';
  const saisi=parseFloat((document.getElementById('ac_montant')||{}).value||0)||0;
  if(!agentId){toast('Choisissez le bénéficiaire','err');return;}
  if(saisi<=0){toast('Saisissez un montant','err');return;}
  const at=AGENT_TYPES[type];
  const agentName=at.agentName(at.getAgent(agentId))||'';
  const d=(document.getElementById('ac_date')||{}).value;
  const tvaMode=(document.getElementById('ac_tva')||{}).value||'ttc';
  const id=uid();
  // Le montant retenu pour le compte est le TTC decaisse, quel que soit le regime saisi
  const k=tvaAcompte({montant:saisi,tvaMode});
  const a={id,nature:'versement',type,agentId,agentName,
    montant:k.ttc,montantSaisi:Math.round(saisi*100)/100,tvaMode,
    ht:k.ht,tva:k.tva,
  ...
  // Justificatif : stocke dans l'espace comptable, protege comme les autres pieces
  const fichier=((document.getElementById('ac_fichier')||{}).files||[])[0];
  if(fichier){
    try{
      const ext=fichier.name.split('.').pop();
      const path=`paiements/acomptes/${id}.${ext}`;
      const{error}=await SUPA.storage.from(ATTACH_BUCKET).upload(path,fichier,{upsert:true});
      if(error)throw error;
      a.filePath=path;a.fileName=fichier.name;
    }catch(e){toast('Acompte enregistré mais justificatif non envoyé : '+(e.message||e),'err');}
  }
  const l=getAcomptes();l.push(a);saveAcomptes(l);
  toast(`Acompte de ${k.ttc.toLocaleString('fr-FR',{minimumFractionDigits:2})} € TTC enregistré ✓`,'ok');
...
function voirCompteAcompte(type,agentId){
  ...
  b+=`<div class="tbl-wrap"><table class="tbl"><thead><tr>
    <th>Date</th><th>Mouvement</th><th>Référence</th><th>TVA</th><th>Justificatif</th><th class="r">Montant TTC</th>
  </tr></thead><tbody>`;
  const libTva={ttc:'TTC',ht:'HT + TVA',sans_tva:'Sans TVA'};
  c.lignes.forEach(a=>{
    const versement=a.nature==='versement';
    const k=versement?tvaAcompte(a):null;
    b+=`<tr>
      ...
      <td style="font-size:.74rem">${versement?`<span style="font-weight:700">${libTva[k.mode]||'TTC'}</span>${k.tva?`<div class="muted" style="font-size:.66rem">HT ${eur(k.ht)} · TVA ${eur(k.tva)}${k.recup?' récup.':''}</div>`:''}`:'<span class="muted">—</span>'}</td>
      <td>${versement&&a.filePath?attachLink(a.filePath,a.fileName||'Voir',''):'<span class="muted" style="font-size:.72rem">—</span>'}</td>
...
function supprimerAcompte(id,type,agentId){
  ...
  modalConfirm('Annuler ce versement ?',
    ...
    async()=>{
      // Le justificatif part avec le versement
      if(a.filePath){try{await SUPA.storage.from(ATTACH_BUCKET).remove([a.filePath]);}catch(e){console.error('[acompte]',e);}}
  ...
  if(FACT_AGENT){
    ...
    if(!rows.length)h+=`<div class="empty"><div class="big">📋</div>Aucun dossier à facturer pour cet agent.</div>`;
      ...
      <div style="display:flex;align-items:center;gap:.6rem;margin-top:1rem;flex-wrap:wrap;background:var(--bg-soft);border-radius:9px;padding:.55rem .85rem">
        <span style="font-size:.78rem;font-weight:700;color:var(--text-mut)">🧾 Régime TVA</span>
        <select id="fact_tva" onchange="FACT_TVA=this.value" style="font-family:'Saira',sans-serif;font-size:.85rem;font-weight:700;border:1.5px solid var(--border-grey);border-radius:8px;padding:.35rem .65rem;background:#fff;outline:none">
          <option value="" ${!FACT_TVA?'selected':''}>Selon chaque dossier</option>
          <option value="ttc" ${FACT_TVA==='ttc'?'selected':''}>TTC — montants TTC, TVA non récupérable</option>
          <option value="ht" ${FACT_TVA==='ht'?'selected':''}>HT — TVA ajoutée, récupérable</option>
          <option value="sans_tva" ${FACT_TVA==='sans_tva'?'selected':''}>Sans TVA — autoentrepreneur</option>
        </select>
        <span style="font-size:.78rem;font-weight:700;color:var(--text-mut);margin-left:.6rem">📅 Date de l'appel à facturation</span>
      ...
      ${(function(){
        const mts=getMailTpl().filter(t=>t.profil==='compta');
        if(!mts.length)return '';
        return `<div style="display:flex;align-items:center;gap:.6rem;margin-top:.5rem;flex-wrap:wrap;background:var(--bg-soft);border-radius:9px;padding:.55rem .85rem">
          <span style="font-size:.78rem;font-weight:700;color:var(--text-mut)">✉️ Modèle d'e-mail</span>
          <select id="fact_tpl" style="font-family:'Saira',sans-serif;font-size:.85rem;font-weight:700;border:1.5px solid var(--border-grey);border-radius:8px;padding:.35rem .65rem;background:#fff;outline:none">
            <option value="">— Message par défaut —</option>
            ${mts.map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join('')}
          </select>
          <span class="muted" style="font-size:.72rem">l'appel à facturation reste joint</span>
        </div>`;
      })()}
...
async function genererFactureAgent(){
  ...
  const lignes=checks.map(c=>{
    ...
    const tvaMode=FACT_TVA||effectiveTvaMode(r,type==='callcenter'?'callcenter':type);
  ...
  if(wantsSend&&agent&&agent.email){
    try{
      // Objet et corps repris d'un modele Comptabilite si l'un est choisi
      const tpl=getMailTpl().find(x=>x.id===((document.getElementById('fact_tpl')||{}).value||''));
      const vars={apf:apfNum,intervenant:agentName,type:at.label,
                  total:total.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2})+' €',
                  societe:(getSettings().company||'EGN Rénovation'),
                  date:dApf.toLocaleDateString('fr-FR')};
      const remplir=t=>String(t||'').replace(/\{\{(\w+)\}\}/g,(m,k)=>vars[k]!=null?vars[k]:m);
      const sujet=tpl&&tpl.subject?remplir(tpl.subject):`${apfNum} — Appel à facturation ${esc(at.label)}`;
      // Le corps du modele precede toujours le document lui-meme
      const corps=tpl&&tpl.body?`<div style="font-family:Arial,sans-serif;font-size:14px;color:#1A2B3D">${esc(remplir(tpl.body)).replace(/\n/g,'<br>')}</div><hr style="border:none;border-top:1px solid #e2e8f0;margin:18px 0">${doc}`:doc;
      await callSendEmail({to:agent.email,profil:'compta',subject:sujet,html:corps});
  ...
  const factureObj={id:factureId,apfNum,type,date:dApf.getTime(),poseurId:agentId,poseurName:agentName,tvaModeApf:FACT_TVA||'',
