...
function collecterEcrituresTva(){
  ...
  segApfDeleg().forEach(a=>{
    ...
    (a.recIds||[]).forEach(id=>{
      ...
      const e={sens:'ca',r,ttc,ht,tva:r2(ttc-ht),piece:a.numApf||'APF délégataire',p:periodeDe(ts),
               intervenant:deleg?deleg.name:'— Délégataire inconnu —',famille:'Délégataire',
               ouvrir:`voirApfDeleg('${a.id}')`};
  ...
  segFactures().forEach(f=>{
    ...
    (f.lignes||[]).forEach(l=>{
      ...
      const at=AGENT_TYPES[f.type||'poseur']||{label:f.type};
      const e={sens:'charge',r,ttc,ht,tva:r2(ttc-ht),piece:f.apfNum||'APF',p:periodeDe(ts),
               intervenant:f.poseurName||'—',famille:at.label,
               ouvrir:`voirApfDossiers('${f.id}')`};
  ...
  getMvts().filter(v=>['entree','reception'].includes(v.sens)).forEach(v=>{
    ...
    const e={sens:'charge',r:null,ttc:r2(ht+tva),ht,tva,
             piece:mat?mat.nom:'Facture fournisseur',p:periodeDe(ts),
             intervenant:v.fournisseurNom||'— Fournisseur inconnu —',famille:'Fournisseur',
             ouvrir:v.facturePath?`openAttachment('${v.facturePath}','')`:''};
    (paye&&e.p?retenues:attente).push(e);
  });
...
function renderComptaTva(){
  ...
  if(retenues.length){
    h+=`<div class="panel"><div class="panel-h"><h3>Écritures — ${esc(libPeriode)}</h3><div class="sp"><span class="muted">${retenues.length} ligne${retenues.length>1?'s':''}</span><button class="btn btn-navy btn-sm" onclick="exportTvaCSV()">📤 Exporter</button></div></div><div class="panel-b"><div class="tbl-wrap"><table class="tbl">
      <thead><tr><th>Sens</th><th>Intervenant</th><th>N° Dossier</th><th>Client</th><th>Pièce</th><th class="r">TTC</th><th class="r">HT</th><th class="r">TVA</th></tr></thead><tbody>`;
    retenues.sort((a,b)=>b.tva-a.tva).forEach(e=>{
      ...
      h+=`<tr ${e.r?`style="cursor:pointer" onclick="ouvrirLigne(event,'${e.r.id}')"`:''}>
        ...
        <td><div style="font-weight:700;font-size:.82rem;color:${ca?'#A32D2D':'var(--purple)'}">${esc(e.intervenant||'—')}</div><div class="muted" style="font-size:.66rem">${esc(e.famille||'')}</div></td>
        ...
        <td>${e.ouvrir
          ?`<a href="javascript:void(0)" onclick="event.stopPropagation();${e.ouvrir}" style="font-size:.76rem;font-weight:700;color:var(--blue);text-decoration:none" title="Ouvrir la pièce">📄 ${esc(e.piece||'—')}</a>`
          :`<span class="muted" style="font-size:.75rem">${esc(e.piece||'—')}</span>`}</td>
...
function exportTvaCSV(){
  ...
  let csv='Sens;Intervenant;Famille;N Dossier;Client;Operation;Piece;Mois;TTC;HT;TVA\n';
  retenues.forEach(e=>{
    ...
    csv+=`${e.sens==='ca'?'Entree':'Sortie'};${e.intervenant||''};${e.famille||''};${e.r?e.r.dossier||'':''};${e.r?clientNameOf(e.r):''};${op?op.name:''};${e.piece||''};${MOIS_FR[e.p.mois]} ${e.p.annee};${nb(e.ttc)};${nb(e.ht)};${nb(e.tva)}\n`;
  });
...
/* Enregistre les deux profils d'expedition e-mail */
function saveProfilsEmail(){
  const lire=cle=>({
    nom:(document.getElementById('em_'+cle+'_nom')||{}).value||'',
    adresse:((document.getElementById('em_'+cle+'_adresse')||{}).value||'').trim().toLowerCase(),
    replyTo:((document.getElementById('em_'+cle+'_reply')||{}).value||'').trim(),
    signature:(document.getElementById('em_'+cle+'_sign')||{}).value||''
  });
  const profils={client:lire('client'),compta:lire('compta')};
  // Controle de forme : une adresse mal saisie ferait echouer tous les envois
  const valide=a=>/^[^@\s]+@[^@\s]+\.[a-z]{2,}$/i.test(a);
  for(const [cle,p] of Object.entries(profils)){
    if(p.adresse&&!valide(p.adresse)){toast(`Adresse d'expédition invalide (profil ${cle})`,'err');return false;}
    if(p.replyTo&&!valide(p.replyTo)){toast(`Adresse de réponse invalide (profil ${cle})`,'err');return false;}
  }
  const s=getSettings();s.emailProfils=profils;save(K.set,s);
  toast('Expéditeurs enregistrés ✓','ok');
  return true;
}
async function testerEmail(){
  const to=((document.getElementById('em_test')||{}).value||'').trim();
  const profil=(document.getElementById('em_test_profil')||{}).value||'client';
  const box=document.getElementById('em_test_res');
  if(!to){if(box)box.innerHTML='<span style="color:var(--coral);font-size:.8rem">Saisissez une adresse de test.</span>';return;}
  if(!saveProfilsEmail())return;   // l'envoi doit utiliser ce qui est a l'ecran
  if(box)box.innerHTML='<span class="muted" style="font-size:.8rem">⏳ Envoi en cours…</span>';
  try{
    const r=await callSendEmail({to,profil,
      subject:"Test d'envoi — "+(getSettings().company||'EGN Rénovation'),
      html:`<p>Ceci est un test d'envoi depuis le CRM.</p><p>Profil utilisé : <b>${profil==='compta'?'Comptabilité':'Clients'}</b>.</p>`});
    if(box)box.innerHTML=`<div style="background:#EFF7E8;border:1.5px solid var(--green);border-radius:9px;padding:.6rem .9rem;font-size:.82rem">
      <b>✅ Message envoyé</b>${r&&r.from?' depuis '+esc(r.from):''} — vérifiez la boîte ${esc(to)}.</div>`;
  }catch(e){
    if(box)box.innerHTML=`<div style="background:#FEE8E8;border:1.5px solid var(--coral);border-radius:9px;padding:.6rem .9rem;font-size:.82rem">
      <b>⛔ Échec</b><div class="muted" style="font-size:.76rem;margin-top:.2rem">${esc(e.message||e)}</div></div>`;
  }
}
