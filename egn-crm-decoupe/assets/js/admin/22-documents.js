/* ============================================================
   ...
   Modèles d e-mail, modèles de documents HTML et PDF, documents admin, paramètres, diagnostic de synchronisation
   ============================================================ */
/* ====================================================================
...
function pdfChampsDisponibles(){
  const groupes=[
    {g:'Client',items:[['raison_sociale','Raison sociale'],['signataire','Nom complet'],['nom','Nom'],['prenom','Prénom'],
      ['fonction','Fonction'],['tel','Téléphone'],['email','Email'],['nom_site','Nom du site'],['siret','SIRET']]},
    {g:'Adresses',items:[['adresse','Adresse'],['cp','Code postal'],['ville','Ville'],['adresse_travaux','Adresse des travaux']]},
    {g:'Dossier',items:[['dossier','N° de dossier'],['operation','Opération'],['zone','Zone'],['date_jour',"Date du jour"]]},
...
function setMailTpl(){
  ...
  else h+=ts.map(t=>`<div class="cfg-item"><div style="flex:1;min-width:0"><div class="nm">${esc(t.name)}</div><div class="muted" style="font-size:.72rem;margin-top:.15rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(t.subject||'(sans objet)')}</div></div><span class="sp"><button class="iconbtn" onclick="openMailTpl('${t.id}')">✏️</button><button class="iconbtn del" onclick="delMailTpl('${t.id}')">🗑️</button></span></div>`).join('');
  h+=`</div></div></div>`;
...
function saveMailTpl(){
  ...
  const ts=getMailTpl();const data={name,subject:document.getElementById('mt_subject').value,body:document.getElementById('mt_body').value};
...
function openMailSend(recId){
  ...
  const ts=getMailTpl();
  const tplOpts=`<option value="">— Message vierge —</option>`+ts.map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join('');
  const docs=r.docs||[];
...
function setSociete(){
  ...
  let h=`<div class="panel"><div class="panel-h"><h3>📱 Notifications WhatsApp</h3><div class="sp">
